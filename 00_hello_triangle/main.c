...
int main () {
	...
	*/
	...
	/* change to 3.2 if on Appble OS X */
	...
	const GLchar* vsb = (const GLchar*)vertex_shader;
	const GLchar* fsb = (const GLchar*)fragment_shader;
	glShaderSource (vs, 1, &vsb, NULL);
	glCompileShader (vs);
	fs = glCreateShader (GL_FRAGMENT_SHADER);
	glShaderSource (fs, 1, &fsb, NULL);
	glCompileShader (fs);
